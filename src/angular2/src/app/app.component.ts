...
export class AppComponent implements OnInit{
  ...
  private stableList: Job[] = [];
  private brokenList:Job[]=[];
  private semistableList: Job [] = [];
...
this.stableList = [];
   ...
   (x) => {
    ...
    for(var i = 0; i < x.length; i++ ){
      ...
      if (data.displayName > numberLastBuild){
        idLastBuild = i;
        numberLastBuild = data.displayName;
      }
    ...
    let data = x[idLastBuild].json();
                    ...
                    if (average <= 0.1){
                      this.stableList.push(new Job(data2.name, data.result, data2.url, data.url, data.displayName));
                    } else if (average<0.8){
                      this.semistableList.push(new Job(data2.name, data.result, data2.url, data.url, data.displayName));
                    }else {
                      this.brokenList.push(new Job (data2.name, data.result, data2.url, data.url, data.displayName));
                    }
